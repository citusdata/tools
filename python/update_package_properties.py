...
def convert_citus_changelog_into_debian_changelog(citus_changelog: str, fancy: bool,
                                                  fancy_version_number: int, microsoft_email: str, name_surname: str,
                                                  changelog_date) -> str:
    lines = citus_changelog.splitlines()
    lines[0] = get_debian_changelog_header(lines[0], fancy, fancy_version_number)
    lines.append(get_debian_trailer(microsoft_email, name_surname, changelog_date))
...
def prepend_latest_changelog_into_debian_changelog(latest_changelog: str, tag_name: str, fancy: bool,
    ...
    debian_latest_changelog = convert_citus_changelog_into_debian_changelog(latest_changelog, fancy,
...
def get_rpm_changelog_history(project_name: str, spec_file_path: str) -> str:
...
def get_rpm_header(project_name: str, microsoft_email: str, name_surname: str, project_version: str,
                   is_fancy_versioned: bool,
                   fancy_version_number: int, changelog_date: date):
    formatted_date = changelog_date.strftime("%a %b %d %Y")
    return f"* {formatted_date} - {name_surname} <{microsoft_email}> {get_version_number_with_project_name(project_name, project_version, is_fancy_versioned, fancy_version_number)} "
...
def update_rpm_spec(project_name: str, project_version: str, microsoft_email: str,
    ...
    content = template.render(version=project_version, fancy_version_str=fancy_version_str,
                              fancy_version_no=fancy_version_number, changelog=changelog)
    with open(spec_file_name, "w+") as writer:
        writer.write(content)


@validate_parameters
def update_all_changes(github_token: non_empty(str), project_name: non_empty(str), project_version: is_version(str),
    ...
    update_rpm_spec(project_name, project_version, microsoft_email, name_surname, fancy, fancy_version_number,
                    spec_file_name, release_date, templates_path)
