...
def update_release(github_token: str, project_name: str, project_version: is_version(str), main_branch: str,
    ...
    if is_major_release(project_version):
        ...
        if not replace_line_in_file(multi_extension_out_path, MULT_EXT_MAJOR_SEARCH_PATTERN,
                                    f" {project_version}"):
        ...
        if not replace_line_in_file(multi_extension_out_path, MULT_EXT_PATCH_SEARCH_PATTERN,
                                    f" {project_version}"):
            raise ValueError(f"{multi_extension_out_path} does not have match for version")
        print(f"### OK {multi_extension_out_path} file is updated with project version {project_version}.### ")
        ...
        print(f"### Committing changes for branch {release_branch_name}...### ")
        run(f'git commit -a -m "{project_name} version to {project_version}"')
        print(f"### OK Changes committed for {release_branch_name}### ")
