...
def update_docker_file_for_latest_postgres(
    ...
    content = process_template_file(
        project_version,
        template_path,
        docker_templates[SupportedDockerImages.latest],
        postgres_version,
    )
...
def update_regular_docker_compose_file(
    ...
    dest_file_name = (
        f"{exec_path}/{docker_outputs[SupportedDockerImages.docker_compose]}"
    )
    write_to_file(content, dest_file_name)
...
def update_docker_file_alpine(
    ...
    content = process_template_file(
        project_version,
        template_path,
        docker_templates[SupportedDockerImages.alpine],
        postgres_version,
    )
...
def update_docker_file_for_postgres12(
    project_version: str, template_path: str, exec_path: str, postgres_version: str
):
    content = process_template_file(
        project_version,
