...
class SupportedProject(Enum):
    citus = ProjectDetails(
        name="citus",
        version_suffix="citus",
        github_repo_name="citus",
        changelog_project_name="citus",
        packaging_branch="all-citus",
    )
    citus_enterprise = ProjectDetails(
        name="citus-enterprise",
        version_suffix="citus",
        github_repo_name="citus-enterprise",
        changelog_project_name="citus-enterprise",
        packaging_branch="all-enterprise",
    )
    pg_auto_failover = ProjectDetails(
        name="pg-auto-failover",
        version_suffix="",
        github_repo_name="pg_auto_failover",
        changelog_project_name="pg_auto_failover",
        packaging_branch="all-pgautofailover",
    )
    pg_auto_failover_enterprise = ProjectDetails(
        name="pg-auto-failover-enterprise",
        version_suffix="",
...
def is_project_changelog_header(header: str):
    ...
    if not re.match(
        r"^### \w+[-]?\w+\sv\d+\.\d+\.\d+\s\(\w+\s\d+,\s\d+\)\s###$", header
    ):
        raise ValueError(
...
class PackagePropertiesParams:
    ...
    def project_name_suffix(self) -> str:
        return (
            self.project.value.version_suffix
            if not self.project.value.version_suffix
    ...
    def rpm_header(self):
        ...
        return (
            f"* {formatted_date} - {self.name_surname} <{self.microsoft_email}> "
...
@validate_parameters
def debian_changelog_header(
    supported_project: SupportedProject,
    project_version: str,
    fancy: bool,
    fancy_version_number: int,
) -> str:
    package_properties_params = PackagePropertiesParams(
    ...
    return (
        f"{supported_project.value.name} ({version_on_changelog}) stable; urgency=low"
    )


def get_debian_latest_changelog(
    package_properties_params: PackagePropertiesParams,
) -> str:
    lines = []
    lines.append(
        debian_changelog_header(
            package_properties_params.project,
...
def prepend_latest_changelog_into_debian_changelog(
    package_properties_params: PackagePropertiesParams, changelog_file_path: str
) -> None:
...
def get_changelog_entry(package_properties_params: PackagePropertiesParams):
    default_changelog_entry = (
        f"Official {package_properties_params.debian_changelog_project_version} release of "
        f"{package_properties_params.changelog_project_name}"
    )
    return (
        package_properties_params.changelog_entry
        if package_properties_params.changelog_entry
...
def update_rpm_spec(
    ...
    if (
        len(history_lines) > 0
        and package_properties_params.version_number_with_project_name
        in history_lines[0]
    ):
        raise ValueError(
    ...
    content = template.render(
        version=package_properties_params.project_version,
        rpm_version=rpm_version,
        project_name=package_properties_params.project.value.name,
        fancy_version_no=package_properties_params.fancy_version_number,
        changelog=changelog,
    )
    with open(
        spec_full_path,
...
def validate_package_properties_params_for_update_all_changes(
    package_props: PackagePropertiesParams,
):
...
def update_all_changes(
    package_properties_params: PackagePropertiesParams, packaging_path: str
):
    validate_package_properties_params_for_update_all_changes(package_properties_params)
    templates_path = f"{BASE_PATH}/templates"
    update_pkgvars(package_properties_params, templates_path, f"{packaging_path}")
...
if __name__ == "__main__":
    ...
    exec_date = (
        datetime.strptime(arguments.date, "%Y.%m.%d %H:%M")
        if arguments.date
    ...
    package_properties = PackagePropertiesParams(
        project=project,
        project_version=prj_ver,
        fancy=fancy,
        fancy_version_number=arguments.fancy_ver_no,
        name_surname=arguments.name,
        microsoft_email=arguments.email,
        changelog_date=exec_date,
        changelog_entry=arguments.changelog_entry,
    )
