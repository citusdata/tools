...
from .common_validations import (
    CITUS_MINOR_VERSION_PATTERN, CITUS_PATCH_VERSION_PATTERN)
...
def prepare_release_branch_for_patch_release(patchReleaseParams: PatchReleaseParams):
    ...
    if patchReleaseParams.is_test:
        ...
        non_test_release_branch = patchReleaseParams.release_branch_name.rstrip(
            "-test")
        ...
        checkout_branch(patchReleaseParams.release_branch_name,
                        patchReleaseParams.is_test)
...
def prepare_upcoming_version_branch(upcoming_params: UpcomingVersionBranchParams):
    ...
    if not upcoming_params.is_test:
        ...
        create_pull_request_for_upcoming_version_branch(upcoming_params.repository, upcoming_params.main_branch,
                                                        ...
                                                        upcoming_params.upcoming_devel_version)
    print(
        f"### Done {upcoming_params.upcoming_version_branch} flow executed. ###")
...
def push_branch(upcoming_version_branch):
    print(
        f"Pushing changes for {upcoming_version_branch} into remote origin... ###")
...
def update_schema_version_in_citus_control(citus_control_file_path, schema_version):
    ...
    if not replace_line_in_file(citus_control_file_path, CITUS_CONTROL_SEARCH_PATTERN,
        ...
        raise ValueError(
            f"{citus_control_file_path} does not have match for version")
    print(
...
def get_current_schema_from_citus_control(citus_control_file_path: str) -> str:
    print(
        f"### Reading current schema version from {citus_control_file_path}... ###")
    ...
    if schema_not_found:
        raise ValueError(
            "Version info could not be found in citus.control file")
...
def update_version_in_configure_in(project_name, configure_in_path, project_version):
    ...
    if not replace_line_in_file(configure_in_path, CONFIGURE_IN_SEARCH_PATTERN,
        ...
        raise ValueError(
            f"{configure_in_path} does not have match for version")
    print(
...
def create_new_sql_for_upgrade_path(current_schema_version, distributed_dir_path,
    ...
    with open(f"{distributed_dir_path}/{newly_created_sql_file}", "w", encoding=DEFAULT_ENCODING_FOR_FILE_HANDLING,
        ...
        content = content + "\n\n"
        content = content + \
            f"-- bump version to {upcoming_minor_version}-1" + "\n\n"
...
def validate_parameters(major_release_flag: bool):
    ...
    if major_release_flag and arguments.earliest_pr_date:
        raise ValueError(
            "earliest_pr_date could not be used for major releases")
...
if __name__ == "__main__":
    ...
    try:
        ...
        main_branch = arguments.main_branch if arguments.main_branch else repo_details[
            arguments.prj_name]["branch"]
        print(
            f"Using main branch {main_branch} for the repo {arguments.prj_name}.")
