...
def upload_to_package_cloud(
    distro_name, package_name, package_cloud_token, repo_name
) -> ReturnValue:
    ...
    with open(package_name, "rb") as file_handle:
        files = {
            ...
            "package[package_file]": (package_name, file_handle),
        }

        package_query_url = f"https://{package_cloud_token}:@packagecloud.io/api/v1/repos/{repo_name}/packages.json"
        print(f"Uploading package {os.path.basename(package_name)}")
        response = requests.post(package_query_url, files=files, timeout=150)
...
def upload_files_in_directory_to_package_cloud(
    directoryName: str,
    distro_name: str,
    package_cloud_token: str,
    repo_name: str,
    current_branch: str,
    main_branch: str,
) -> MultipleReturnValue:
    ...
    for file in files:
        if file.endswith((".rpm", ".deb")):
            ret_val = upload_to_package_cloud(
                distro_name, file, package_cloud_token, repo_name
            )
...
def delete_package_from_package_cloud(
    package_cloud_token: str,
    repo_owner: str,
    repo_name: str,
    distro_name: str,
    ...
    delete_url = (
        f"https://{package_cloud_token}:@packagecloud.io/api/v1/repos/{repo_owner}/{repo_name}/"
        f"{distro_name}/{distro_version}/{package_name}"
    )

    response = requests.delete(delete_url, timeout=150)
    return ReturnValue(
        response.ok, response.content, package_name, distro_name, repo_name
    )
...
def package_exists(
    ...
    response = requests.get(
        query_url, auth=HTTPBasicAuth(package_cloud_token, ""), timeout=60
    )
    return response.ok
...
if __name__ == "__main__":
    ...
    multiple_return_value = upload_files_in_directory_to_package_cloud(
    ...
    for rv in multiple_return_value.return_values:
        if not rv.success_status:
            ...
            print(
                f"File successfully uploaded. Distro: {rv.distro} File Name: {os.path.basename(rv.file_name)} "
                f"Repo Name: {rv.repo}"
            )
