...
def test_build_packages():
    ...
    signing_credentials = SigningCredentials(
        secret_key, TEST_GPG_KEY_PASSPHRASE)
    input_output_parameters = InputOutputParameters.build(PACKAGING_EXEC_FOLDER, BASE_OUTPUT_FOLDER,
                                                          output_validation=False)

    build_packages(GH_TOKEN, PLATFORM, BuildType.release,
                   signing_credentials, input_output_parameters, is_test=True)
...
def test_get_required_package_count():
    assert get_required_package_count(
        PACKAGING_EXEC_FOLDER, platform="el/8") == 9
