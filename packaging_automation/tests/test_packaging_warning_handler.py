...
def test_parse_ignore_lists():
    base_ignore_list, debian_ignore_list, rpm_ignore_list = parse_ignore_lists(
...
def test_rpm_filter_warning_lines():
    with open(f"{TEST_BASE_PATH}/files/packaging_warning/sample_warning_build_output_rpm.txt", "r") as reader:
        ...
        assert len(base_warning_lines) == 11 and len(package_specific_warning_lines) == 1
...
def test_validate_output_rpm():
    with open(f"{TEST_BASE_PATH}/files/packaging_warning/sample_warning_build_output_rpm.txt", "r") as reader:
        ...
        with pytest.raises(ValueError):
            validate_output(output,
                            f"{TEST_BASE_PATH}/files/packaging_warning/packaging_ignore.yml", PackageType.rpm)
