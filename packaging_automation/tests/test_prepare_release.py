...
BASE_PATH = pathlib2.Path(__file__).parents[2] if os.getenv(
    "BASE_PATH") is None else os.getenv("BASE_PATH")
...
def initialize_env() -> str:
    ...
    if not os.path.exists(test_base_path_major):
        run(
            f"git clone https://github.com/citusdata/citus.git {test_base_path_major}")
...
def test_major_release():
    ...
    update_release_return_value = update_release(
        ...
        earliest_pr_date=datetime.strptime(
            '2021.03.25 00:00', '%Y.%m.%d %H:%M'),
        exec_path=test_base_path_major, is_test=True)

    run(f"git checkout {update_release_return_value.release_branch_name}")

    assert file_includes_line(test_base_path_major,
                              MULTI_EXTENSION_OUT, " 10.1.0")
    assert file_includes_line(test_base_path_major,
                              CONFIGURE_IN, "AC_INIT([Citus], [10.1.0])")
    assert file_includes_line(test_base_path_major,
                              CONFIGURE, "PACKAGE_VERSION='10.1.0'")
    assert file_includes_line(test_base_path_major,
                              CONFIGURE, "PACKAGE_STRING='Citus 10.1.0'")
    ...
    assert file_includes_line(test_base_path_major,
                              CONFIGURE, "PACKAGE_VERSION='10.1.0'")
    assert get_last_commit_message(
        test_base_path_major) == "Bump citus version to 10.1.0\n"

    run(f"git checkout {update_release_return_value.upcoming_version_branch}")

    assert file_includes_line(test_base_path_major,
                              CITUS_CONTROL, "default_version = '10.2-1'")
    ...
    assert file_includes_line(test_base_path_major,
                              MULTI_EXTENSION_OUT, " 10.2devel")

    assert count_line_in_file(test_base_path_major, MULTI_EXTENSION_SQL,
                              "ALTER EXTENSION citus UPDATE TO '10.2-1';") == 2
    assert file_includes_line(test_base_path_major,
                              CONFIG_PY, "MASTER_VERSION = '10.2'")
    assert file_includes_line(test_base_path_major,
                              CONFIGURE_IN, "AC_INIT([Citus], [10.2devel])")
    assert file_includes_line(test_base_path_major,
                              CONFIGURE, "PACKAGE_VERSION='10.2devel'")
    assert file_includes_line(test_base_path_major,
                              CONFIGURE, "PACKAGE_STRING='Citus 10.2devel'")
    assert file_includes_line(test_base_path_major, CONFIGURE,
    ...
    assert get_last_commit_message(
        test_base_path_major) == 'Bump citus version to 10.2devel\n'
    run(f"git checkout {MAIN_BRANCH}")
