...
def test_docker_statistics_collector():
    ...
    session = Session()
    fetch_and_store_docker_statistics("citus", db_parameters=db_params, is_test=True,
                                      test_day_shift_index=test_day_shift_index)
    first_day = datetime.today() + timedelta(days=test_day_shift_index)
    first_day_record = session.query(DockerStats).filter_by(
        stat_date=first_day.date()).first()
    fetch_and_store_docker_statistics("citus", db_parameters=db_params, is_test=True,
    ...
    second_day_record = session.query(DockerStats).filter_by(
        stat_date=second_day.date()).first()
    third_day_record = session.query(DockerStats).filter_by(
        stat_date=third_day.date()).first()

    pull_count_diff = third_day_record.total_pull_count - \
        first_day_record.total_pull_count
