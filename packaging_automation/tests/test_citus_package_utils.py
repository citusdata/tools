...
TEST_BASE_PATH = os.getenv(
    "BASE_PATH", default=pathlib2.Path(__file__).parents[2])
...
INPUT_OUTPUT_PARAMETERS = InputOutputParameters.build(PACKAGING_EXEC_FOLDER, OUTPUT_FOLDER,
...
def test_get_signing_credentials():
    ...
    assert signing_credentials.secret_key == "verysecretkey" and signing_credentials.passphrase == "123"
    ...
    expected_gpg_key = get_private_key_by_fingerprint_without_passphrase(
        fingerprints[0])
    delete_all_gpg_keys_by_name(TEST_GPG_KEY_NAME)
    assert signing_credentials.secret_key == transform_key_into_base64_str(
        expected_gpg_key) and signing_credentials.passphrase == TEST_GPG_KEY_PASSPHRASE
...
def test_delete_rpm_key_by_name():
    ...
    assert TEST_GPG_KEY_NAME not in output.stdout.decode(
        "ascii") and output.returncode == 1
...
def test_get_postgres_versions():
    ...
    assert release_versions == [
        "11", "12", "13"] and nightly_versions == ["12", "13", "14"]
...
def test_sign_packages():
    ...
    input_output_parameters = InputOutputParameters.build(PACKAGING_EXEC_FOLDER, f"{OUTPUT_FOLDER}",
                                                          output_validation=False)
    sign_packages(sub_folder="centos-8", signing_credentials=signing_credentials,
                  input_output_parameters=input_output_parameters)
    sign_packages(sub_folder="debian-stretch", signing_credentials=signing_credentials,
                  input_output_parameters=input_output_parameters)
