...
def default_changelog_param_for_test(changelog_date):
    changelog_param = PackagePropertiesParams(
        project=SupportedProject.citus,
...
def test_get_version_number_with_project_name():
    assert (
        DEFAULT_CHANGELOG_PARAM_FOR_TEST.version_number_with_project_name
        == "10.0.3.citus-1"
    )
...
def verify_prepend_debian_changelog(changelog_file_path):
    with open(
        ...
        content = reader.read()
    with open(
        f"{TEST_BASE_PATH}/files/verify/debian_changelog_with_10.0.3.txt",
        "r",
        encoding=DEFAULT_ENCODING_FOR_FILE_HANDLING,
        errors=DEFAULT_UNICODE_ERROR_HANDLER,
    ) as reader:
...
def test_rpm_changelog():
    ...
    with open(
        f"{TEST_BASE_PATH}/files/verify/rpm_latest_changelog_reference.txt",
        "r",
        encoding=DEFAULT_ENCODING_FOR_FILE_HANDLING,
        errors=DEFAULT_UNICODE_ERROR_HANDLER,
    ) as reader:
...
def test_update_pkg_vars():
    ...
    try:
        update_pkgvars(
            DEFAULT_CHANGELOG_PARAM_FOR_TEST, templates_path, f"{TEST_BASE_PATH}/files/"
        )
